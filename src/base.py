...
def baseline(model = LinearSVC(), ):
    ...
    # %%
    histogram = histogram_building(train_text, bag=3)
    num_feature = len(histogram) # 12887
    tokens = list(histogram.index)
    print("dimension = ", num_feature)
    train_data,num_train = text_preprocessing(train_text,tokens,num_feature, bag=3)
    val_data,num_val = text_preprocessing(val_text,tokens,num_feature, bag=3)
    test_data,num_test = text_preprocessing(test_text,tokens,num_feature, bag=3)
    # %%
